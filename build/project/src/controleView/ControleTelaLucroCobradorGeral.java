...
public class ControleTelaLucroCobradorGeral extends Application implements Initializable {
	
	public static Stage palcoTelaLucroCobradorGeral = null;
	...
	public void eventoBtnRelatorio(ActionEvent event) {
		...
		try {
			...
			fachada.relatorioComissaoCobradorEntregador2(dataIni, dataFim, "relatorios/recebimento_geral2.jasper");
			//fachada.relatorioComissaoCobradorEntregador2(dataIni, dataFim, "relatorios/testeFest.jasper");
			//fachada.relatorioComissaoCobradorEntregador2(dataIni, dataFim, "relatorios/vendedorRelatorioLucro.jasper");
	...
	public void eventoBtnPesquisar(ActionEvent event) {
		...
		ArrayList<Operacao> listarCobradores = new ArrayList();


		try {
			listarCobradores = fachada.getCobradoresGeral(dataIni, dataFim);
...
//lembrar muda op de tela fechamento
	public void start(Stage palcoTelaLucroCobradorGeral) throws Exception {
		this.palcoTelaLucroCobradorGeral = palcoTelaLucroCobradorGeral;
		
		
		try {	
			// Origem do arquivo FXML da tela
			Parent origemTela = FXMLLoader.load(getClass().getResource("/view/TelaRelatorioDiarioGeral.fxml"));
			...
			palcoTelaLucroCobradorGeral.setScene(cena);
			
			palcoTelaLucroCobradorGeral.setTitle("Recebimento Geral");
			
			palcoTelaLucroCobradorGeral.getIcons().add(new Image("file:resources/images/address_book_32.png"));
			
			palcoTelaLucroCobradorGeral.setResizable(false);

			palcoTelaLucroCobradorGeral.setOnCloseRequest(new EventHandler<WindowEvent>() {
		          public void handle(WindowEvent we) {
		              //System.out.println("Stage is closing");
		        	  ControleTelaInicial.setCodRelatorioGeral(0);
		          }
		      });        
			palcoTelaLucroCobradorGeral.close();

			palcoTelaLucroCobradorGeral.show();
